...
function initEvenHandles() {
  ...
  htmlElements.input.addEventListener('blur', textInputRed); // я добавил
  htmlElements.addButton.addEventListener('click', addButtonRedBorder);
}
...
function addButtonClicked() {
  ...
  if (!checkIfColorCanBeAdded(textValue)) {
    alert ("There's no such a color");
  }
  else {
    if (checkIfColorAdded(textValue)){
      alert('Color has been already added');
    }
    else {
      addColor(textValue);
      chooseColor(textValue);
      setColor(textValue);
    }
...
function removeColorClicked() {
  ...
  let textValueAdded = document.getElementById('textInput').value;
  let selectedValueDropdown = document.getElementById ('select').value;
  if (textValueAdded == ""){
    alert('Please enter a value first');
  }
  else {
    removeColor(textValueAdded);
    reset();
    setColor('--color', selectedValueDropdown);
  }
}
...
function removeSelectedColorClicked() {
  ...
  if (getSelectedValue() == "not selected") {
    ...
    removeColor(selectedValueDropdown);
    setColor('--color', selectedValueDropdown);
  }
}
...
function textInputRed() {
  let textInputRed = document.getElementById('textInput');
  let textValueInputForBorder = document.getElementById('textInput').value;
    if (!checkIfColorCanBeAdded(textValueInputForBorder) || checkIfColorAdded(textValueInputForBorder)) {
      textInputRed.classList.add('invalid');
      textInputRed.classList.remove('input-color');
    }
    else {
      textInputRed.classList.remove('invalid');
      textInputRed.classList.add('input-color');
    }
};

function addButtonRedBorder() {
  let textValueInput = document.getElementById('textInput').value;
  let textInputRedColor = document.getElementById('textInput');
  let dig = /\d{1}/;
  let regExpNumber = dig.test(textValueInput);
    if (textValueInput === '' || textValueInput === ' '  || regExpNumber) {
      textInputRedColor.classList.add('invalid');
      textInputRedColor.classList.remove('input-color');
    }
    else {
      textInputRedColor.classList.remove('invalid');
      textInputRedColor.classList.add('input-color');
    }
};